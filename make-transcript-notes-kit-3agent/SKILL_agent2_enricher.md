...
- The title / header matching the input section — **clean topic title only** (e.g., "Logistic Regression", not "Logistic Regression — Enriched Lecture Notes" or "Enriched by Agent 2"). No pipeline jargon, no agent names, no phase labels, no callout legends, no enrichment source attribution.
